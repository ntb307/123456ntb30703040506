...
<body>
 ...
 document.getElementById('adminLogin').style.display = 'none';
    ...
    // 비밀번호 확인
    기능. checkAdminPassword() {
 constinputPass = document.getElementById('adminPasswordInput').가치;
 만약 (입력 통과 === ADMIN_PASSWARD) {
        // 비밀번호가 맞으면 모달 닫고 adminSection 보이기
 AdminLogin() 닫기;
 document.getElementById('adminSection').style.display = '차단';
      } 또 다른 {
 alert("비밀번호가 올바르지 않습니다.");
      }
    }
 ...
 // 행 단위로 나누기 (줄바꿈 기준)
 const 행 = rawText.split(/\\r?\n/);

 // 각 행을 탭(\t)으로 나누어 2차원 배열 형태 구성
 const dataMatrix = 행.map(행 => {
 반환 행.split(/\\t/;
 });
 ...
 미리보기Table.innerHTML = HTML;
 }

 // 테이블 데이터를 서버로 전송하여 저장 (POST /saveSheet)
 비동기 함수 saveSheetData() {
 const pasteArea = document.getElementById('pasteArea');
 constrawText = pasteArea.value.trim();
 만약 (!rawText) {
 alert("붙여넣은 데이터가 없습니다.");
 반환;
 }
 ...
 const 행 = rawText.split(/\\r?\n/);
 // 2차원 배열
 const dataMatrix = lows.map(row => low.split(/\\t/));

 시도해보기 {
 const response = wait fetch('/saveSheet', {
 방법: 'POST',
 헤더: { '콘텐츠 유형': '애플리케이션/json'},
 본문: JSON.stringify({ 데이터: dataMatrix })
 });
 만약 (응답) {
 alert("데이터가 성공적으로 저장되었습니다.");
 } 그 외 {
